...
interface DeliveryVan {
  ...
  battery: number;
  parcels: number;
  status: 'ACTIVE' | 'OVERLOADED' | 'CRITICAL_BATTERY' | 'RESCUING';
  location: string;
  isByzantine: boolean;
  lastUpdate: number;
}
...
class HackathonDemo {
  ...
  private logEvent(message: string, type: 'info' | 'success' | 'warning' | 'error' = 'info'): void {
  ...
  private simulateFoxMQMessage(topic: string, payload: any, qos: number): number {
  ...
  private simulateConsensusRound(): number {
  ...
  private detectCriticalConditions(): void {
  ...
  private triggerRescueOperation(van: DeliveryVan): void {
    ...
    if (availableVans.length > 0) {
      const rescuer = availableVans[Math.floor(Math.random() * availableVans.length)];
      ...
      const latency = this.simulateFoxMQMessage('swarm/rescue', {
        ...
        rescuingVan: rescuer.id,
        parcelsToTransfer: Math.min(10, van.parcels)
      }, 2); // QoS 2 for rescue coordination
      
      this.logEvent(`🆘 RESCUE: ${rescuer.id} responding to ${van.id} (${latency.toFixed(2)}ms)`, 'success');
      
      // Complete rescue after delay
      setTimeout(() => {
        this.completeRescue(van, rescuer);
      }, 5000);
    }
  }

  private completeRescue(requestingVan: DeliveryVan, rescuingVan: DeliveryVan): void {
  ...
  private simulateByzantineAttack(): void {
